...
export default async function handler(
  ...
  const targetUrl = `${transactionsService.apiUrl}/title-companies/${id}`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Proxy to Transactions Service (title-companies) failed for ${req.method} ${targetUrl}:`, response.status, errorData);
    ...
    console.error(`API route /api/title-companies/${id} error:`, error);
