...
export default async function handler(
  ...
  if (id === 'today' || id === 'overdue') {
    // Let the specific route handlers deal with these
    return res.status(404).json({ error: 'Route not found. Use /api/tasks/today or /api/tasks/overdue' });
  }
  
  const transactionsService = getTransactionsServiceConfig();
  const targetUrl = `${transactionsService.apiUrl}/tasks/${id}`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Proxy to Transactions Service (tasks) failed for ${req.method} ${targetUrl}:`, response.status, errorData);
    ...
    console.error(`API route /api/tasks/${id} error:`, error);
