...
export default async function handler(
  ...
  try {
    ...
    const targetUrl = `${transactionsService.apiUrl}/automation${queryString}`;
    ...
    if (!response.ok) {
      console.error(`Proxy to Transactions Service (automation) failed for ${req.method} ${targetUrl}:`, response.status, maybeJson);
