...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const targetUrl = `${baseUrl}/imports/leads/commit`;
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorData = { error: errorText || 'Commit failed', status: response.status };
      }
    ...
    console.error('API route /api/imports/leads/commit error:', error);
