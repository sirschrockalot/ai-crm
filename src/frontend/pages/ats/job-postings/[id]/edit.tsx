...
const EditJobPostingPage: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      router.push(`/ats/job-postings/${id}`);
    } catch (error: any) {
      toast({
        title: 'Error',
          ...
          <VStack align="stretch" spacing={6}>
            <HStack>
              <IconButton
                icon={<FiArrowLeft />}
                aria-label="Back"
                onClick={() => router.push(`/ats/job-postings/${id}`)}
              />
              <VStack align="start" spacing={1} flex={1}>
                <Heading size="lg">Edit Job Posting</Heading>
                <Text color="gray.600">Update job details and requirements</Text>
              </VStack>
              <Button
                colorScheme="blue"
                onClick={handleSave}
